...
function buildLayout(tree: DiagramSemanticNode[], expandedIds: Set<string>) {
  ...
  const place = (node: DiagramSemanticNode, depth: number, path: DiagramSemanticNode[]): LayoutNode => {
    const nextPath = [...path, node];
...
export function DiagramExplorer({ tree, searchQuery, onSearchChange, onSelectLabel }: DiagramExplorerProps) {
  const { t } = useI18n();
  ...
  const activeNode = layout.nodes.find((node) => node.node.id === activeHit?.node.id) ?? layout.nodes[0] ?? null;
  const activePathIds = new Set(activeHit?.path.map((node) => node.id) ?? []);
  const q = searchQuery.trim().toLowerCase();
  const searchHits = q ? allHits.filter((hit) => hit.node.label.toLowerCase().includes(q)).slice(0, 8) : [];
  ...
  const fit = useCallback(() => {
    ...
    const next = clampScale(Math.min(vp.clientWidth / layout.width, vp.clientHeight / layout.height) * 0.92);
    setScale(next);
    setPan({ x: 24, y: 24 });
        ...
        <div className="relative grid min-h-[min(78vh,760px)] lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 border-b border-border/60 lg:border-b-0 lg:border-r">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 bg-background/75 px-4 py-3 backdrop-blur-sm">
              <div className="min-w-0">
                ...
                <p className="truncate text-sm font-medium text-foreground">{activeHit.node.label}</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex rounded-md border border-border/60 bg-background/75 p-0.5">
                  <Button type="button" variant={mode === "graph" ? "secondary" : "ghost"} size="sm" className="h-8" onClick={() => setMode("graph")}>
                    {t("diagram.graph")}
                  </Button>
                  <Button
                    type="button"
                ...
                <div className="flex rounded-md border border-border/60 bg-background/75 p-0.5">
                  <Button type="button" variant="ghost" size="sm" className="h-8 w-8 px-0" onClick={() => zoomBy(0.86)} aria-label={t("diagram.zoomOut")}>
                    -
                  </Button>
                  ...
                  <Button type="button" variant="ghost" size="sm" className="h-8 w-8 px-0" onClick={() => zoomBy(1.16)} aria-label={t("diagram.zoomIn")}>
                    +
                  </Button>
                </div>
                ...
                <Button type="button" variant="outline" size="sm" className="h-8" onClick={() => setExpandedIds(collectIds(tree))}>
                  {t("diagram.openAll")}
                </Button>
                <Button type="button" variant="outline" size="sm" className="h-8" onClick={() => setExpandedIds(new Set(activeHit.path.map((node) => node.id)))}>
                  Focus
                </Button>
              </div>
            </div>
            ...
            {q ? (
              <div className="border-b border-border/60 bg-background/85 px-4 py-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{t("diagram.searchResults")}</p>
                  <button type="button" className="rounded-md border border-border/60 px-2 py-1 text-[10px] text-muted-foreground hover:bg-muted/50 hover:text-foreground" onClick={() => onSearchChange("")}>
                    {t("diagram.clear")}
                  </button>
                </div>
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {searchHits.length > 0 ? searchHits.map((hit) => (
                    ...
                    >
                      <span className="mb-1 block text-[10px] text-muted-foreground">{t("diagram.openPath")}</span>
                      <span className="line-clamp-2">{hit.node.label}</span>
                    </button>
                  )) : <span className="text-xs text-muted-foreground">{t("diagram.noNodeFound")}</span>}
                </div>
              </div>
            ) : null}
            ...
            <div
              ref={viewportRef}
              className={cn("relative h-[min(72vh,650px)] overflow-hidden", dragging ? "cursor-grabbing" : "cursor-grab")}
          ...
          <aside className="bg-background/80 p-4 backdrop-blur-sm">
            <div className="rounded-lg border border-border/60 bg-muted/20 p-4">
              ...
              <h4 className="text-lg font-semibold leading-tight text-foreground">{activeHit.node.label}</h4>
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                <div className="rounded-md border border-border/60 bg-background/55 p-2">
                  <span className="block text-[10px] uppercase tracking-wider">{t("diagram.branches")}</span>
                  <span className="mt-1 block text-sm font-semibold text-foreground tabular-nums">{activeHit.node.children.length}</span>
                </div>
                <div className="rounded-md border border-border/60 bg-background/55 p-2">
                  <span className="block text-[10px] uppercase tracking-wider">{t("diagram.descendants")}</span>
                  <span className="mt-1 block text-sm font-semibold text-foreground tabular-nums">{countDescendants(activeHit.node)}</span>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                <Button type="button" variant="outline" size="sm" onClick={() => openPath(activeHit.path.slice(0, 1))}>
                  {t("diagram.root")}
                </Button>
                <Button type="button" variant="secondary" size="sm" disabled={activeHit.node.children.length === 0} onClick={() => toggleNode(activeHit.node)}>
            ...
            <div className="mt-4 rounded-lg border border-border/60 bg-muted/15 p-3">
              <p className="mb-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{t("diagram.path")}</p>
                ...
                {activeHit.path.map((node, index) => (
                  <button
                    ...
                    className={cn(
                      ...
                      node.id === activeId ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-background/65 hover:text-foreground"
                    )}
            ...
            {activeHit.node.children.length > 0 ? (
              <div className="mt-4 rounded-lg border border-border/60 bg-muted/15 p-3">
                <p className="mb-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{t("diagram.branches")}</p>
                <div className="space-y-1.5">
        ...
        return (
          ...
          >
            <span className="flex h-full gap-3 p-3">
              <span className={cn("h-full w-2 shrink-0 rounded-full", active ? "bg-primary" : inPath ? "bg-primary/75" : "bg-muted-foreground/45")} />
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-2 flex items-center gap-2 text-[10px] text-muted-foreground">
                  <span>{hasChildren ? `${item.node.children.length} ${labels.branches}` : labels.final}</span>
...
function SequenceView({
  ...
  return (
    <div className="relative" style={{ width, height: 520 }}>
      <svg className="absolute inset-0" width={width} height={520} viewBox={`0 0 ${width} 520`} aria-hidden="true">
        <defs>
        ...
        <line x1="130" y1="260" x2={width - 150} y2="260" stroke="url(#sequence-link)" strokeWidth="4" />
      </svg>
      {nodes.map((node, index) => {
        ...
        return (
          ...
          >
            ...
            <span className="line-clamp-3 text-xs font-semibold leading-snug text-foreground">{truncate(node.label, 70)}</span>
          </button>
        );
      })}
    </div>
