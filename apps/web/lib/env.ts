...
const EnvSchema = z.object({
  ...
  DEFAULT_TENANT_ID: z.string().min(3).max(64).regex(/^[a-z0-9-]+$/).default("default-tenant"),
  ...
  KEYCLOAK_ISSUER_URL: z.preprocess((value) => (value === "" ? undefined : value), z.string().url().optional()),
  ...
  for (const key of ["KEYCLOAK_ISSUER_URL", "KEYCLOAK_CLIENT_ID", "KEYCLOAK_CLIENT_SECRET"] as const) {
    if (!env[key]) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: [key],
