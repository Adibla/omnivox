...
export async function getJobById(jobId: string): Promise<DbPipelineJob | null> {
  await initializeDatabase();
  const db = getDbPool();
  const { rows } = await db.query(`${jobSelectSql()} where external_id = $1 and deleted_at is null`, [jobId]);
...
export async function getJobByIdempotency(idempotencyKey: string): Promise<DbPipelineJob | null> {
  await initializeDatabase();
  const db = getDbPool();
  const { rows } = await db.query(`${jobSelectSql()} where idempotency_key = $1 and deleted_at is null`, [idempotencyKey]);
  return rows.length === 0 ? null : hydrateJob(rowToJob(rows[0]));
}

export async function listJobsByOwner(input: { tenantId: string; ownerIssuer: string; ownerSubject: string; limit?: number }) {
  await initializeDatabase();
  const db = getDbPool();
...
export async function softDeleteJob(jobId: string) {
  await initializeDatabase();
  const db = getDbPool();
  await db.query(`update pipeline_jobs set deleted_at = now(), updated_at = now() where external_id = $1`, [jobId]);
}

export async function updateJobState(jobId: string, state: DbPipelineJob["state"], error?: string | null) {
  await initializeDatabase();
  const db = getDbPool();
  await db.query(
...
export async function upsertActionStatus(input: { jobId: string; actionId: string; status: ActionStatus }) {
  await initializeDatabase();
  const db = getDbPool();
...
async function upsertActionStatusWithDb(db: Queryable, input: { internalJobId: number; actionId: string; status: ActionStatus }) {
  await db.query(
  ...
  await db.query(`update pipeline_jobs set updated_at = now() where id = $1`, [input.internalJobId]);
}
...
async function persistResultEntities(db: Queryable, internalJobId: number, rawResult: AnalysisOutput) {
...
async function replaceActions(db: Queryable, internalJobId: number, actions: AnalysisOutput["actions"]) {
  ...
  for (const [index, action] of actions.entries()) {
    ...
    await db.query(
      ...
      [internalJobId, actionId, action.title, action.owner, action.dueDate ?? null, action.priority, action.risk, action.actionType, index]
    );
