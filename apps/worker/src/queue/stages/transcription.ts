import { getAudioFormatFromContentType, getAudioFormatFromFileName, type PipelineMessage, type TranscriptSegment } from "@omnivox/shared";
...
async function transcribeFromStorage(
  ...
  const extension = getAudioFormatFromFileName(objectKey) ?? getAudioFormatFromContentType(contentType) ?? "ogg";
...
export async function processTranscription(payload: PipelineMessage, context: StageContext) {
  ...
  await context.pipelineQueue.add(
    ...
    { jobId: buildQueueJobId([payload.jobId, "preprocess"]), attempts: 3, backoff: { type: "exponential", delay: 500 } }
  );
}
